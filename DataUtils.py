...
def get_labels(csv_fname, limit=None, interval=1, start=0, labels=['person']):
...
def get_differences(csv_fname, OBJECT, limit=None, interval=1, delay=15):
    ...
    return np.array([1 if sym_diff(labels[i-delay], labels[i]) else 0 for i in xrange(delay,interval)])
